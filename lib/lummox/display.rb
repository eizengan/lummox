...
class Lummox::Display
  ...
  def self.all
    size = Lummox::SDLError.raise_if(:negative?) { Lummox::SDL::Core.get_num_video_displays }
  ...
  def name
    Lummox::SDLError.raise_if(:nil?) { Lummox::SDL::Core.get_display_name(@display_index) }
  end

  def bounds
    bounds = Lummox::SDL::Core::Rect.new
    Lummox::SDLError.raise_if(:negative?) { Lummox::SDL::Core.get_display_bounds(@display_index, bounds) }
  ...
  def display_modes
    size = Lummox::SDLError.raise_if(:negative?) { Lummox::SDL::Core.get_num_display_modes(@display_index) }
    Array.new(size) do |mode_index|
      display_mode = Lummox::DisplayMode.new
      Lummox::SDLError.raise_if(:negative?) do
        Lummox::SDL::Core.get_display_mode(@display_index, mode_index, display_mode.sdl_display_mode)
      end
      display_mode
    end
  end
  ...
  def desktop_display_mode
    display_mode = Lummox::DisplayMode.new
    Lummox::SDLError.raise_if(:negative?) do
      Lummox::SDL::Core.get_desktop_display_mode(@display_index, display_mode.sdl_display_mode)
    end
    display_mode
  end
end
